...
;                  código
		...
		EXPORT PortH_Output			; Permite chamar PortH_Output de outro arquivo
		EXPORT PortJ_Input          ; Permite chamar PortJ_Input de outro arquivo
		EXPORT PortC_Input          ; Permite chamar PortC_Input de outro arquivo
			
		EXPORT inicializa_LCD
		EXPORT mostra_LCD
		EXPORT verifica_teclado		
...
; ****************************************
	...
	ORR     R1, #GPIO_PORTH					;Seta o bit da porta H, fazendo com OR
...
EsperaGPIO
	...
	ORR     R2, #GPIO_PORTH					;Seta o bit da porta H, fazendo com OR
	...
	LDR     R0, =GPIO_PORTH_AHB_AMSEL_R		;Carrega o R0 com o endereço do AMSEL para a porta H
    STR     R1, [R0]					    ;Guarda no registrador AMSEL da porta H da memória
	...
	LDR     R0, =GPIO_PORTH_AHB_PCTL_R      ;Carrega o R0 com o endereço do PCTL para a porta H
    STR     R1, [R0]                        ;Guarda no registrador PCTL da porta H da memória
	...
	LDR     R0, =GPIO_PORTH_AHB_DIR_R		;Carrega o R0 com o endereço do DIR para a porta H
	MOV     R1, #2_00001111					;PH0, PH1, PH2 e PH3 para o motor de passo
    STR     R1, [R0]						;Guarda no registrador
	...
	MOV     R1, #2_01000111					;PM0, PM1 e PM2 para o LCD
    STR     R1, [R0]						;Guarda no registrador
    ...
    STR     R1, [R0]                        ;Escreve na porta
	LDR     R0, =GPIO_PORTH_AHB_AFSEL_R     ;Carrega o endereço do AFSEL da porta H
    STR     R1, [R0]                        ;Escreve na porta
	...
	LDR     R0, =GPIO_PORTH_AHB_DEN_R		;Carrega o endereço do DEN
    MOV     R1, #2_00001111                 ;Ativa os pinos PH0, PH1, PH2 e PH3 como I/O Digital
    STR     R1, [R0]						;Escreve no registrador da memória funcionalidade digital
    ...
    MOV     R1, #2_01000111                 ;Ativa os pinos PM0, PM1 e PM2 como I/O Digital
    STR     R1, [R0]						;Escreve no registrador da memória funcionalidade digital
...
; Escrever função que ativa ou desativa os transistores
; ****************************************
	LDR	R1, =GPIO_PORTH_AHB_DATA_R		    ;Carrega o valor do offset do data register
											;Read-Modify-Write para escrita
	LDR R2, [R1]
...
; ****************************************
	LDR	R1, =GPIO_PORTM_DATA_R		    ;Carrega o valor do offset do data register
											;Read-Modify-Write para escrita
	LDR R2, [R1]
	BIC R2, #2_01000111                     ;Primeiro limpamos o bit do lido da porta R2 = R2 & 00001000
	ORR R0, R0, R2                          ;Fazer o OR do lido pela porta com o parâmetro de entrada
	STR R0, [R1]                            ;Escreve na porta P o barramento de dados do pino P5
	BX LR									;Retorno
; -------------------------------------------------------------------------------
